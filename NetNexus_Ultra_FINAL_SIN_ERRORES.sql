-- NetNexus Ultra v1.5.1 - Base de Datos FINAL
-- Versión libre de errores y warnings
...
-- Limpiar tablas existentes (orden correcto por dependencias)
...
-- TABLA UBICACION (CON FUNCIONALIDAD DE FAVORITAS)
...
CREATE INDEX idx_ubicacion_favorita ON ubicacion(es_favorita);
CREATE INDEX idx_tarjeta_cliente ON tarjetas_usuario(Cliente_idCliente);

-- VERIFICACIÓN FINAL
SELECT 'NetNexus Ultra v1.5.1 - Base de Datos creada exitosamente!' as RESULTADO;
SELECT COUNT(*) as TABLAS_CREADAS FROM information_schema.tables WHERE table_schema = 'netnexus_ultra';
SELECT COUNT(*) as CLIENTES_REGISTRADOS FROM cliente;
SELECT COUNT(*) as TECNICOS_DISPONIBLES FROM tecnicos;
SELECT COUNT(*) as TIPOS_SERVICIO FROM tiposervicio;
SELECT COUNT(*) as CONTRATOS_ACTIVOS FROM contrato WHERE estado = 'activo';
SELECT COUNT(*) as UBICACIONES_FAVORITAS FROM ubicacion WHERE es_favorita = TRUE;

-- Consulta de prueba del sistema de ubicaciones favoritas
SELECT 
    'Ubicaciones Favoritas por Cliente:' as REPORTE,
    c.nombre,
    c.apellido,
    u.direccion,
    u.ciudad,
