...
public class UserContractsWindow extends JFrame {
    ...
    private void loadUserContracts() {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            String query = """
                SELECT c.idContrato, c.tiposervicio, c.monto_total, c.fecha_inicio, c.fecha_fin
                FROM contrato c
                ...
                try (ResultSet rs = pstmt.executeQuery()) {
                    ...
                    while (rs.next()) {
                        ...
                        JPanel contractCard = createContractCard(
                            ...
                            rs.getBigDecimal("monto_total").doubleValue(),
                            rs.getDate("fecha_inicio"),
                            rs.getDate("fecha_fin"),
                            "Activo", // Estado fijo ya que filtramos por fecha vigente
                            rs.getString("tiposervicio") // Usar el mismo tipo de servicio
                        );
    ...
    private void cancelContract(int contractId, String planTipo) {
        ...
        if (confirmacion == JOptionPane.YES_OPTION) {
            try (Connection conn = DatabaseConnection.getConnection()) {
                String updateQuery = "UPDATE contrato SET fecha_fin = CURDATE() WHERE idContrato = ?";
    ...
    private void showContractDetails(int contractId) {
        try (Connection conn = DatabaseConnection.getConnection()) {
            String query = """
                SELECT c.*, 'Internet/Telefonía' as tipo_servicio, 'Servicios de conectividad' as descripcion
                FROM contrato c
                WHERE c.idContrato = ?
                """;
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(query)) {
                ...
                try (ResultSet rs = pstmt.executeQuery()) {
                    if (rs.next()) {
                        ...
                        details.append("═══════════════════════\n\n");
                        details.append("ID Contrato: ").append(rs.getInt("idContrato")).append("\n");
                        details.append("Plan: ").append(rs.getString("tiposervicio")).append("\n");
                        details.append("Precio: $").append(rs.getBigDecimal("monto_total")).append("\n");
