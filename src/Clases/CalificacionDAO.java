...
public class CalificacionDAO {
    ...
    public static List<String[]> obtenerServiciosParaCalificar(int clienteId) {
        ...
        String sql = "SELECT s.idServicios, ts.nombre, s.descripcion, s.fecha_servicio, " +
                    "ts.idTipoServicio " +
                    "FROM servicios s " +
                    "JOIN contrato c ON s.Contrato_idContrato = c.idContrato " +
                    "JOIN tiposervicio ts ON s.TipoServicio_idTipoServicio = ts.idTipoServicio " +
                    "WHERE c.Cliente_idCliente = ? AND s.estado = 'completado' " +
                    "AND s.idServicios NOT IN (" +
                    "   SELECT Servicio_idServicio FROM calificaciones_servicios " +
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                String[] servicio = {
                    String.valueOf(rs.getInt("idServicios")),
                    rs.getString("nombre"),
                    rs.getString("descripcion"),
                    rs.getDate("fecha_servicio").toString(),
                    String.valueOf(rs.getInt("idTipoServicio"))
                };
                servicios.add(servicio);
            }
            
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error al obtener servicios para calificar", e);
        }
    ...
    public static List<String[]> obtenerEstadisticasServicios() {
        ...
        String sql = "SELECT * FROM estadisticas_servicios ORDER BY promedio_general DESC";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                String[] estadistica = {
                    ...
                    String.valueOf(rs.getDouble("promedio_general")),
                    String.valueOf(rs.getDouble("promedio_tecnico")),
                    String.valueOf(rs.getDouble("promedio_servicio")),
                    String.valueOf(rs.getDouble("promedio_tiempo")),
                    String.valueOf(rs.getDouble("promedio_precio")),
            ...
            logger.log(Level.SEVERE, "Error al obtener estadísticas de servicios", e);
        }
